...
#problems: during the first iteration the program seems to magically delete a value, for no apparent reason. the program also seems to change some values again for no apparent reason.
import numpy as np
globalcomparer = []
def Nestedcheck(height, width, heightcap, widthcap):
	global globalcomparer
	print height,width, heightcap, widthcap
	if len(height) != 1:
